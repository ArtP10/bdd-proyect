...
const handleLogin = async () => {
  ...
  try {
    ...
    if (data.data.user_role === 'Cliente' || form.user_type === 'Cliente') {
             router.push('/client/dashboard');
        } else {
             alert(`Bienvenido ${data.data.user_name}. Rol: ${data.data.user_role}`);
...
.title { font-size: 1.5rem; font-weight: 700; color: #333; margin-bottom: 1.5rem; }
...
.radio-item input { margin-right: 5px; accent-color: #e91e63; }
